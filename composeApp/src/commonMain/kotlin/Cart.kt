...
import com.common.models.CartItem
import com.common.navigation.Screen
import com.common.payment.fetchPaymentIntentClientSecret
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
...
fun Cart(
    ...
    LaunchedEffect(amount) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val secret = fetchPaymentIntentClientSecret(amount)
                ...
                Box(
                    ...
                    Row(
                        ...
                        Button(
                            shape = RoundedCornerShape(24.dp),
                            onClick = {
                                if (paymentIntentClientSecret == null) return@Button
                            ...
                            Text(if (isLoading) "Loading..." else "Checkout")
                        }
                    }
                }
            }
    ...
    Box(
        ...
        Row(
            ...
            Column {
                Text("₹${item.price}")
                Text("x ${item.quantity}")
                val totalItemPrice = (item.price * item.quantity * 100).toInt() / 100.0
                Text(
                    "$totalItemPrice",
                    fontSize = 15.sp,
                    fontWeight = FontWeight.Bold,
